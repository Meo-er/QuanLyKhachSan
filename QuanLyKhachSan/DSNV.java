...
public class DSNV {
    ...
    public void them(NhanVien nvMoi) {
        if (maTonTai(nvMoi.getMaNV())) {
            System.out.println("Ma " + nvMoi.getMaNV() + " da ton tai, khong them vao danh sach. ");
        } else {
            dsnv = Arrays.copyOf(dsnv, dsnv.length + 1);
            dsnv[soluong] = nvMoi;
            soluong++;
    ...
    public void docFile() {
        File file = new File("QuanLyKhachSan/NhanVien.txt");
        ...
        try (Scanner sc = new Scanner(file)) {
            while (sc.hasNextLine()) {
                ...
                if (parts.length == 5) {
                    double luong = Double.parseDouble(parts[4]);
                    NhanVien nv = new NhanVien(parts[0], parts[1], parts[2], parts[3], luong);
                    dsnv = Arrays.copyOf(dsnv, dsnv.length + 1);
                    dsnv[dsnv.length - 1] = nv;
                    soluong++;
                }
